<div align="center">
  <img src="docs/favicon.svg" width="80" height="80" alt="Python Django Boilerplate"/>
</div>
...
If you want to run Django directly on your machine (debug Option 2), you also need:

| Tool | Version | Purpose | Download |
|---|---|---|---|
| **Python** | 3.12+ | Runs the Django app and installs dependencies | [python.org](https://www.python.org/downloads/) |
...
> `AWS_SQS_ENDPOINT_URL` and `SQS_QUEUE_URL` are injected by Docker Compose automatically for the `app` and `worker` containers. Add them to `.env.local` only when running the Django app directly on the host (debug Option 2), replacing `localstack` with `localhost`.
...
| Django app (direct) | http://localhost:5000 | Bypass Nginx for debugging |
...
> Code changes are reflected immediately without rebuilding — the source directory is mounted as a volume and uvicorn's hot-reload reloads on file changes.
...
`postman_collection.json` at the repo root covers every endpoint over both transports:

| Folder | Requests |
|---|---|
| **REST** | Health Check · Register · Login · Refresh Token · Upload File · Publish Event · List Events · Cache Ping · Cache Test · Cache Delete |
| **GraphQL** | Register · Login · Refresh Token · Upload File · Get Signed URL · Publish Event · List Events · Cache Ping · Cache Test · Cache Delete · Health Check |

**How to use:**

1. In Postman, click **Import** and select `postman_collection.json`.
2. The collection has four variables pre-set: `base_url` (`http://localhost`), `access_token`, `refresh_token`, `media_id`.
3. Run **Login** — a test script captures `access_token` and `refresh_token` automatically.
4. Run **Upload File** — a test script captures `media_id` automatically.
5. All subsequent requests use those variables — no manual copy-paste needed.
...
> GraphQL field names use **camelCase** (`accessToken`, `mediaId`, `expiresIn`) even though Python uses snake_case — graphene-django converts them automatically.
...
All external traffic enters through **Nginx on port 80** (`nginx/nginx.conf`). The Django app is not exposed publicly — Nginx proxies to it internally at `app:5000`.
...
```
Django app   →  SQS queue  →  Lambda (locally: worker container)  →  PostgreSQL
```
...
```python
# In a route handler — access via request.app.state
...
If `REDIS_URL` is not set, `request.app.state.cache` is `None` and all cache calls are skipped — the app degrades gracefully without errors.
...
| Non-root containers | Django app runs as `app` system user; Lambda runs as `nobody` — no root access if a container is compromised |
| ECS task role scoped to send-only SQS | Django app can only `SendMessage`. Only the Lambda worker can `ReceiveMessage` and `DeleteMessage` |
...
└── app/
    ...
    ├── integration/   — DRF APIClient + SQLite in-memory via pytest-django (< 2 s total)
...
> **Note:** All workflows are currently set to `workflow_dispatch` only — they will not trigger automatically on push or pull request. To re-enable automatic triggers, update the `on:` block in each workflow file as described below.
...
### `ci.yml` — manual trigger only (originally runs on every push and pull request)
...
#    docker build -t <registry>/django-boilerplate-dev/app:latest .
#    docker push <registry>/django-boilerplate-dev/app:latest
...
The `app` service runs Django under `debugpy` (port 5678) and mounts the source directory as a volume, so code changes are reflected immediately without rebuilding.
...
3. Select **"Docker: Attach to Django"** and press **F5**.
...
Run only the infrastructure in Docker and the Django app directly on your machine. This can give a faster feedback loop.
...
2. Select **"Local: Django Debug"** and press **F5**.

VSCode automatically:
- Starts Postgres, LocalStack, and runs migrations before launch (`preLaunchTask`)
- Launches Django under `debugpy` on port 5000 with hot reload enabled
- Stops the infrastructure containers when you stop the debugger (`postDebugTask`)

**Note on hostnames:** when Django runs on the host it cannot resolve Docker service names like `postgres` or `localstack`. The launch config overrides those env vars automatically:
...
The Django app exposes a Prometheus-format metrics endpoint at `http://localhost:5000/metrics`. Prometheus scrapes it every 15 seconds. Query directly at `http://localhost:9090`.
...
> `django-prometheus` is wired via `PrometheusBeforeMiddleware` / `PrometheusAfterMiddleware` in `MIDDLEWARE` and exposes `/metrics` automatically.
...
```logql
{app="django-boilerplate"}                    # all logs
{app="django-boilerplate", level="error"}      # errors only
{app="django-boilerplate"} |= "upload"        # text search
```
